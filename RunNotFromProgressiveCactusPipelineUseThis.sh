...
usage() {
    cat <<'EOF'
Usage:
  bash RunNotFromProgressiveCactusPipelineUseThis.sh --input /path/to/aln1.maf /path/to/aln2.maf ... --reference REF --pre output_prefix --threads 60 --common_workers 15 --genome_path genome.txt --Anc_name AncX --ModelFile tryMLstartree.mod [options]
...
Notes:
  ...
  * The code files are expected to be located in the same directory as this script:
      ...
      - sep_file/1.sh
  * The consensus MAF/FASTA outputs are written at --pre.
  * Additional ancestor-inference intermediates are created under a tmp_XXXXXX directory
...
do
    ...
    if [[ -z "$input_dir" ]]; then
        input_dir="$file_dir"
    elif [[ "$file_dir" != "$input_dir" ]]; then
        echo "Error: all input MAF files must be in the same directory."
        echo "  First input directory: $input_dir"
...
pre_dir_arg=$(dirname "$output_prefix_abs")
pre_base=$(basename "$output_prefix_abs")
mkdir -p "$pre_dir_arg"
...
############################
# 14. Filter genome_path to taxa present in the final alignment and build .info.txt files
############################
...
############################
# 15. Add ancestor row names to the final consensus MAF
############################
python "$script_dir/add_ance2mafwithN.py" "$output_maf_abs" "$ancestor_maf_abs" "$anc_name"

############################
# 16. Check chromosome coverage for each aligned taxon against the ancestor-augmented MAF
############################
for tax in "${taxa_args[@]}"; do
    info_abs="$info_tmp_dir/${tax}.info.txt"
...
############################
# 17. Convert ancestor-augmented MAF to HAL
############################
...
############################
# 18. Build the temporary model file for phyloFit/ancestorsML
############################
...
############################
# 19. Infer ancestral states and write ancestor FASTA
############################
...
############################
# 20. Delete intermediate temporary workspace
############################
rm -rf "$work_dir"
