...
usage() {
    cat <<'USAGEEOF'
Usage:
  bash RunNotFromProgressiveCactusPipelineUseThis.sh --input /path/to/aln1.maf /path/to/aln2.maf ... --reference REF --pre output_prefix --threads 60 --common_workers 15 --genome_path genome.txt [options]
...
Notes:
  * This script is for MAF files not generated by Progressive Cactus.
  ...
  * Input MAF files are first preprocessed:
      1) duplicate sequences within a block are removed;
      2) blocks without the reference are discarded;
      3) the reference row is moved to the first row;
      4) MAFs are split by reference sequence ID;
      5) reference strand and coordinate order are normalized.
  * The consensus MAF/FASTA outputs are written at --pre.
  * Ancestor inference is done per reference chromosome, and the chromosome-level
    ancestor sequences are merged into the final ancestor FASTA.
...
############################
# 15. Infer ancestor HAL/FASTA branches
...
############################
# 16. Delete or preserve intermediate temporary workspace
############################
