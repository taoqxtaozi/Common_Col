...
usage() {
  cat <<'USAGE'
Usage:
  bash make_plan2.sh --tree '(H,(D,(C,E,G,Y)),A,B,((M,J),(N,(O,P))));' --reference H --paths path.txt --threads 60 --common_workers 15 [options]
  bash make_plan2.sh --tree-file input.tre --reference H --paths path.txt --threads 60 --common_workers 15 [options]
...
One of:
  ...
  --tree-file         File containing one Newick tree
