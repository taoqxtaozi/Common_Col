...
#
# 关键点：
# - 两种随机模型：--model uniform / yule
# - RF 阈值：--rf-threshold ∈ (0,1]，默认 1。=1 走 strict；0~1 之间走 relax(=该阈值)。
# - Triplet：内置等价算法（不调用 triplet_dist）
# - 结果输出后，额外打印 tree_1 vs tree_2 的 triplet 距离（raw + normalized）
# - 【修复】占位符替换：逐 token、逐边界替换 + 兜底检查，杜绝 T1/T2... 遗留
